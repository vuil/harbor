...
func TestPermissions(t *testing.T) {
	...
	for index, user := range lwUsers {
		imageVersion := data.GetImageVersion()
		index = (index + 1) % len(lwUsers)
		projectImages := data.UserProjectMapping[lwUsers[index].Username]
		t.Log("Pushing to projects with inadequate permissions. Adding permissions and retying!")
		for _, projectImage := range projectImages {
			var projectName = projectImage.ProjectReq.ProjectName
			imgPushed := instance.PushImage(user, projectName, imageVersion.ImageName, imageVersion.TagName)
			if !imgPushed {
				t.Logf("Could not push to project %s using user %s\n", projectName, user.Username)
			}
			addedPermmissions := instance.AddPermissions(user, projectName)
			if !addedPermmissions {
				t.Errorf("User %s could not be granted permissions for the project %s\n", user.Username, projectName)
			}
			imgPushed = instance.PushImage(user, projectName, imageVersion.ImageName, imageVersion.TagName)
			if !imgPushed {
				t.Errorf("Could not push to project %s after granting permissions to user %s\n", projectName, user.Username)
			}
			t.Logf("Image push to project %s succeded\n", projectName)
		}
	}
}

var (
	shortest = flag.Bool("shortest", false, "run shortest LW-Harbor persistence tests")
...
func TestMain(m *testing.M) {
	...
	if *large {
		...
		pTestParams = testingParams["shortest"]
	} else {
